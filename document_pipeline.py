...
from db_service import initialize_db, User_Auth_Table, Documents, Chunks, upload_file_to_gridfs
...
class DocumentPipeline:
        ...
        try:
            ...
            for file_path in files:
                try:
                    ...
                    print(
                        f"   ❌ Error hashing {os.path.basename(file_path)}: {e}")
                    continue

            return file_hashes

        except Exception as e:
            print(f"❌ Error calculating directory hashes: {e}")
            return {}
    ...
    def format_chatbot_summary(self, namespace_status: Dict, file_inventory: List[Dict] = None) -> str:
        ...
        if not namespace_status['exists']:
            return "❌ No chatbot found in this namespace"

        # Extract namespace name (remove user_id suffix)
        namespace_name = namespace_status.get('namespace_name', 'Unknown')

        summary = f"""
🎯 EXISTING CHATBOT DETECTED!

📊 Chatbot Details:
   🏷️  Name: "{namespace_name}"
   📁 Files: {namespace_status['document_count']} documents
   📝 Chunks: {namespace_status['chunk_count']} total chunks
   🧠 Embeddings: {namespace_status['embedded_chunk_count']}/{namespace_status['chunk_count']} complete
   📅 Created: {namespace_status['created_date'].strftime('%Y-%m-%d %H:%M:%S') if namespace_status['created_date'] else 'Unknown'}
   🔄 Last Updated: {namespace_status['last_updated'].strftime('%Y-%m-%d %H:%M:%S') if namespace_status['last_updated'] else 'Unknown'}

{'✅ This chatbot is ready to use!' if namespace_status['is_complete'] else '⚠️  This chatbot is incomplete (missing embeddings)'}
"""

        if file_inventory:
            summary += "\n📋 Files in this chatbot:\n"
            for file_info in file_inventory:
                status_icon = "✅" if file_info['is_complete'] else "⚠️"
